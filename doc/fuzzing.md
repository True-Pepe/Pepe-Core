# Fuzzing TruePepe Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing TruePepe Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
Every single pull request submitted against the TruePepe Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make TruePepe Core more robust.
...
with certain assembly code from TruePepe Core's code. See [developer notes on sanitizers](/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing TruePepe Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing TruePepe Core using [`afl-fuzz`](https://github.com/google/afl):
