TruePepe Core version 0.13.2 is now available from:

  <https://download.truepepe.org/truepepe-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a truepepe
...
ECDSA signatures inside TruePepe transactions now use validation using
...
Previous versions of TruePepe Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of TruePepe Core was to raise the default minimum
relay fee.

TruePepe Core 0.13.2 will have a strict maximum size on the mempool. The
...
TruePepe Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running truepeped without having to do any manual configuration.
...
TruePepe Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. TruePepe Core will relay transactions with
...
entirely in TruePepe Core 0.13, and it is at this time undecided whether the
...
TruePepe Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
TruePepe Core automatically creates a hidden service to listen on, without
manual configuration. TruePepe Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if TruePepe Core is listening, and
...
TruePeped can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals TruePepe
Core to use floating fees. By default, TruePepe Core will use floating
fees.
...
At all times, TruePepe Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TPC.
Furthermore, TruePepe Core will never create transactions paying less than
...
`truepepe.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 220927 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 220927 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /truepeperpc>
        ProxyPass http://127.0.0.1:20927/
        ProxyPassReverse http://127.0.0.1:20927/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass truepeped rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< truepeperpc:<password>
    </Location>
...
Restarting truepeped will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in truepepe.conf
...
truepepe-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/truepepe-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The TruePepe Core code base has
...
- `truepepe-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `truepepe-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the TruePepe scripting system that in combination with
...
TruePepe transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of TruePepe Core.
...
The two phases are distinct in the TruePepe-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming TruePepe wallet
  software more difficult and which seriously complicates the design of smart
  contracts for TruePepe.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each TruePepe block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of truepepes they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of truepepes being spent is exactly
...
- **Increased security for multisig:** TruePepe addresses (both P2PKH addresses
  ...
  believed to be provided by TruePepe's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of TruePepe's current ledger (called the UTXO set),
  ...
  note that TruePepe Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  TruePepe users to individually opt-in to almost any change in the TruePepe
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and TruePepe Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the TruePepe
client to TruePepe Core 0.13.2 to signal for segwit support. When a
...
Since TruePepe Core 0.10.0, nodes have defaulted to only relaying and
...
- `truepepe-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `truepepe-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
