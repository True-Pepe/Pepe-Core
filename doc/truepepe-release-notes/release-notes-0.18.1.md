TruePepe Core version 0.18.1 is now available from:

  <https://download.truepepe.org/truepepe-0.18.1/>
...
`/Applications/TruePepe-Qt` (on Mac) or `truepeped`/`truepepe-qt` (on
Linux).
...
TruePepe Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use TruePepe Core on unsupported systems.
...
10.10. Additionally, TruePepe Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:20927:20927` (this is an extra `:20927` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@truepepe.org) the TruePepe Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 TPC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in TPC per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/truepepe-conf.md)
  about the `truepepe.conf` file describes how to use it to configure
  TruePepe Core.

- A new document introduces TruePepe Core's BIP174 [Partially-Signed
  TruePepe Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent TruePepe-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  TruePepe Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when TruePepe Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of TruePepe Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart truepeped with the `-deprecatedrpc=generate`
...
- A new `truepepe-wallet` tool is now distributed alongside TruePepe
...
This section describes planned changes to TruePepe Core that may affect
other TruePepe software and services.

- Since version 0.16.0, TruePepe Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with TruePepe Core 0.20 (expected about
  a year after 0.18), TruePepe Core will default to native segwit
  ...
  to bech32 addresses, and if the TruePepe Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in TruePepe Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any TruePepe Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, TruePepe Core
...
- A sub-project of TruePepe Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with TruePepe Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to TruePepe Core's own implementation, although entropy
  gathered by TruePepe Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves TruePepe Core a
...
- On macOS, TruePepe Core now opts out of application CPU throttling
...
- #20927 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
