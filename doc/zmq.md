...
The TruePepe Core daemon can be configured to act as a trusted "border
router", implementing the truepepe wire protocol and relay, making
...
The ZeroMQ feature in TruePepe Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building truepeped:
...
For instance:
    ...
    $ truepeped -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/truepeped.tx.raw \
...
These options can also be provided in truepepe.conf.
...
From the perspective of truepeped, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into truepeped directly. Furthermore, no information is
...
using. TruePeped appends an up-counting sequence number to each
