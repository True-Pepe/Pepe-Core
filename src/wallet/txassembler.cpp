    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional TPC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes TPC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::TPC_TO_TPC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_tpc);
        }
    } else {
        // First try to construct a TPC-to-TPC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::TPC_ONLY;
        ...
        // If TPC-to-TPC fails, create a peg-out transaction
